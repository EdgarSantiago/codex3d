...
test('CLI identity prefixes describe Codex3D instead of Claude Code', () => {
  expect(getCLISyspromptPrefix()).toContain('Codex3D')
  ...
  for (const prefix of CLI_SYSPROMPT_PREFIXES) {
    expect(prefix).toContain('Codex3D')
    expect(prefix).not.toContain('Claude Code')
    expect(prefix).not.toContain("Anthropic's official CLI for Claude")
  }
})

test('simple mode identity describes Codex3D instead of Claude Code', async () => {
  process.env.CLAUDE_CODE_SIMPLE = '1'

  const prompt = await getSystemPrompt([], 'gpt-4o')

  expect(prompt[0]).toContain('Codex3D')
  expect(prompt[0]).not.toContain('Claude Code')
  expect(prompt[0]).not.toContain("Anthropic's official CLI for Claude")
})

test('built-in agent prompts describe Codex3D instead of Claude Code', () => {
  expect(DEFAULT_AGENT_PROMPT).toContain('Codex3D')
  ...
  expect(generalPrompt).toContain('Codex3D')
  ...
  expect(explorePrompt).toContain('Codex3D')
  ...
  expect(guidePrompt).toContain('Codex3D')
  expect(guidePrompt).toContain('You are the Codex3D guide agent.')
  expect(guidePrompt).toContain('**Codex3D** (the CLI tool)')
