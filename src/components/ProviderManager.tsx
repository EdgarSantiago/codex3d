...
function CodexOAuthSetup({
  ...
  return (
    <Box flexDirection="column" gap={1}>
      ...
      <Text>
        Sign in with your ChatGPT account in the browser. Codex3D will store
...
export function ProviderManager({ mode, onDone }: Props): React.ReactNode {
  ...
  function buildCodexOAuthActivationMessage(options: {
    ...
    if (options.warnings.length > 0) {
      return `${options.prefix}. Codex3D switched to it for this session with warnings: ${options.warnings.join('; ')}.`
    }

    return `${options.prefix}. Codex3D switched to it for this session.`
  }
