...
Manage the Codex3D project wiki stored in .openclaude/wiki.
...
function formatInitResult(result: Awaited<ReturnType<typeof initializeWiki>>): string {
  const lines = [`Initialized Codex3D wiki at ${result.root}`]
...
function formatStatus(status: Awaited<ReturnType<typeof getWikiStatus>>): string {
  if (!status.initialized) {
    return `Codex3D wiki is not initialized in this project.\n\nRun /wiki init to create ${status.root}.`
  }
...
function formatIngestResult(
  ...
  return [
    `Ingested ${result.sourceFile} into the Codex3D wiki.`,
    '',
    `Title: ${result.title}`,
